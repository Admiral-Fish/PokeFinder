...
void PIDtoIVs::setupModels()
{
    ...
    auto data = [=](int column) {
        return ui->tableView->model()->data(ui->tableView->model()->index(ui->tableView->currentIndex().row(), column));
    };
...
{
    PokeFinderCore::RNGCache cache(PokeFinderCore::Method::Method1);
...
void PIDtoIVs::calcMethodChannel(u32 pid)
{
    ...
    for (const auto &pair : seeds)
    {
        PokeFinderCore::XDRNGR backward(pair.first);
        u16 sid = backward.nextUShort();
        u32 seed = backward.nextUInt();

        PokeFinderCore::XDRNG forward(seed, 1);
...
{
    QStringList ivs;
    PokeFinderCore::PokeRNG rng(iv1);
    u16 iv2;
