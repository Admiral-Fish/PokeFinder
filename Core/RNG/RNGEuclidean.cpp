...
RNGEuclidean::RNGEuclidean(Method method)
{
    if (method == Method::Channel)
    {
        ...
        // possible Using the first and last calls we can produce a modified adder and multiplier
        // Mult:j = Mult^j
        // Add:j = Add * (1 + Mult + ... + Mult^(j-1))
        // Using j = 4 and XDRNG gives Mult = 0x284A930D and Add = 0xa2974c77
...
std::vector<std::pair<u32, u32>> RNGEuclidean::recoverLower16BitsIV(u8 hp, u8 atk, u8 def, u8 spa, u8 spd, u8 spe) const
{
    ...
    for (u32 k = 0; k <= kmax; k++, t += 0x80000000)
    {
        if ((t % sub1) < 0x10000)
        {
            ...
            origin.push_back(std::make_pair(fullFirst, fullSecond));
        }
    }
    return origin;
}
