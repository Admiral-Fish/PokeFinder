...
#include <Core/Gen8/Generators/UgGenerator8.hpp>
...
#include <Forms/Models/Gen8/UndergroundModel.hpp>
#include <QMenu>
#include <QMessageBox>
#include <QSettings>
...
bool Underground::hasProfiles() const
{
...
void Underground::setupModels()
{
    ...
    ui->filter->disableControls(Controls::GenderRatio | Controls::EncounterSlots);
    ...
    connect(ui->comboBoxProfiles, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &Underground::profilesIndexChanged);
    connect(ui->pushButtonGenerate, &QPushButton::clicked, this, &Underground::generate);
    connect(ui->comboBoxLocation, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &Underground::locationIndexChanged);
    connect(ui->tableView, &QTableView::customContextMenuRequested, this, &Underground::tableViewContextMenu);
...
void Underground::updateLocations()
{
    encounters = Encounters8::getEncounters(Encounter::GrandUnderground, 0, *currentProfile);
    ...
    {
        ui->comboBoxLocation->addItem(QString::fromStdString(locations[index]), index + 2);
    }
}

void Underground::updatePokemon()
{
    auto area = encounters[ui->comboBoxLocation->currentData().toInt() - 2];
...
void Underground::profilesIndexChanged(int index)
{
    if (index >= 0)
    {
...
void Underground::tableViewContextMenu(QPoint pos)
{
    if (model->rowCount() > 0)
    {
