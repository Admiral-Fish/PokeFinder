            ...
            do
            {
                ...
                case Lead::SuctionCups:
                    if ((nextRNG % 25) == nature)
                    {
                        if (grass)
                        {
                            encounterSlot = EncounterSlot::jSlot(nextRNG2 % 100, encounter);
                        }
                        else
                        {
                            levelRand = nextRNG2;
...
std::vector<WildSearcherState4> WildSearcher4::searchPokeRadar(u8 hp, u8 atk, u8 def, u8 spa, u8 spd, u8 spe, u8 index) const
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        if (shiny)
        {
            ...
            if ((lead == Lead::CuteCharmF || lead == Lead::CuteCharmM) && cuteCharm && rng.nextUShort<false>(3) != 0)
            {
                if (!cuteCharmCheck(info, pid))
                {
                    continue;
                }
            }
